...
Description:
    ...
    Uses PyQuil with a QR-decomposition-based unitary for amplitude embedding.
...
Training pipeline (mini-batch mode):
    ...
    Per batch:
      ...
      2. Compute the batch loss (NN.pdf eq. 3, sum over batch events only).
...
classifier_script = textwrap.dedent(f"""\
    # classifier.py
    # Standalone classifier — generated by PQC_HEP_training_file_QR.py
    # Run timestamp  : {timestamp}
    # Circuit        : [{circuit_number}] {circuit_description}
    # Layers         : {n_layers}  |  Parameters: {N_PARAMS}
    # Requires: pyquil (QVM running), scipy, numpy
