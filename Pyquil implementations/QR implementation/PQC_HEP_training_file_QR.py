...
Usage:
    python PQC_HEP_training_file_QR.py
    python PQC_HEP_training_file_QR.py --circuit-number 5 --batch-size 50
...
Arguments (all optional):
    ...
    --sig            : signal filename or full path
    --bkg            : background filename or full path
...
Amplitude embedding:
    A random 16x16 complex matrix M is constructed with the normalised amplitude
    vector as its first column. A QR factorisation (scipy.linalg.qr) gives a
    unitary Q such that Q[:,0] equals the amplitude vector up to a global phase
    (which is physically unobservable). Single LAPACK call — no iterative loop.
...
Training pipeline (mini-batch mode):
    ...
    Per batch:
      ...
      2. Compute the batch loss (weighted binary cross-entropy, sum over batch events only).
...
# Note: this implementation uses a custom 16x16 unitary gate (QR-decomposition) that
# cannot be directly executed on real hardware — native-gate decomposition is needed first.
...
parser = argparse.ArgumentParser(
    description="PQC mini-batch training for HEP classification (QR). "
...
# QUANTUM PIPELINE
# =============================================================================
# Defines all quantum operations: QR-decomposition amplitude embedding (16x16
# unitary via scipy.linalg.qr), measurement, loss, and gradient via the
...
# --- Amplitude embedding via QR decomposition --------------------------------

def build_embedding_unitary(amplitudes):
    """
    Build a 16x16 unitary Q via QR decomposition.
    A random complex matrix M is formed with amplitudes as its first column.
    QR factorisation gives Q with Q[:,0] = ±amplitudes (the sign is a global
    phase, physically unobservable). Single LAPACK call, no iterative loop.
    """
    n    = len(amplitudes)
    M    = np.random.randn(n, n) + 1j * np.random.randn(n, n)
    M[:, 0] = amplitudes.astype(complex)
    Q, _    = qr(M)
    return Q
...
def embed_state_in_pyquil(amplitudes):
    """
    Return (gate_def, embed_instr) for the QR-based embedding of amplitudes.
    Gate applied with reversed qubit order so qubit 0 is the LSB.
    """
...
def run_circuit_and_measure(gate_def, embed_instr, theta):
    """Build a fresh Program from the embedding, apply variational form, run Nm shots, return P(qubit_0=|1>)."""
    p  = Program()
    p += gate_def
    p += RESET()
    p += embed_instr
...
def compute_batch_gradient_and_loss(batch_embeddings, batch_labels, batch_weights, theta,
    ...
    Forward pass gives p_current (used for both loss and analytical dL/dp).
    ...
    p_current = np.array([
        run_circuit_and_measure(gd, ei, theta) for gd, ei in batch_embeddings
    ])
...
# TRAINING SETUP
# =============================================================================
# Pre-computes one QR-decomposition embedding per event (16x16 unitary), initialises
# theta, and creates the sklearn Adam optimiser.


# Pre-compute one QR embedding per event (16x16 unitary + instruction).
# All N_iter passes reuse these cached embeddings, eliminating repeated construction.
print(f"Pre-computing QR embeddings ({N_EVENTS} events)...")
all_embeddings = []
...
if global_do_start or batch_do_start:
    ...
    if batch_do_start:
        p_start = np.array([
            ...
            for gd, ei in tqdm(all_embeddings[:batch_size], desc="  Batch  [start]", unit="event", leave=False)
        ])
...
classifier_script = textwrap.dedent(f"""\
    # classifier.py  —  standalone classifier generated by PQC_HEP_training_file_QR.py
    #
    ...
    #
    # Note: the QR-decomposition embedding uses a custom 16x16 DefGate.
    ...
    #
    # Requirements: pyquil (QVM running), scipy, numpy
    ...
    def build_embedding_unitary(amplitudes):
        # Build a 16x16 unitary with amplitudes as first column via QR factorisation.
        # A random matrix M is constructed with amplitudes as its first column, then
        # scipy.linalg.qr gives a unitary Q with Q[:,0] = amplitudes (up to global phase).
        n          = len(amplitudes)
        M          = np.random.randn(n, n) + 1j * np.random.randn(n, n)
        M[:, 0]    = amplitudes.astype(complex)
        Q, _       = scipy_qr(M)
    ...
    def embed_state_in_pyquil(amplitudes):
        # Apply the 16x16 QR-decomposition unitary via a PyQuil DefGate, encoding the amplitude vector.
        unitary    = build_embedding_unitary(amplitudes)
        gate_def   = DefGate("AMPLITUDE_EMBED", unitary)
        EMBED_GATE = gate_def.get_constructor()
        p  = Program()
