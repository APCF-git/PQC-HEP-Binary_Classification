...
Usage:
    python PQC_HEP_training_file_pytket.py
    python PQC_HEP_training_file_pytket.py --circuit-number 5 --batch-size 50

    All arguments optional; defaults come from USER SETTINGS below.

Arguments (all optional):
    --circuits-file  : path to the circuits library Python file
    --circuit-number : integer 1-19
    --n-layers       : integer >= 1
    --n-iter         : integer >= 1, full passes over all events
    --batch-size     : events per mini-batch (must evenly divide total events)
    --data-folder    : folder containing CSV files
    --sig / --bkg    : signal / background filename or full path
    --max-events     : maximum total events to use

    Data file format (CSV, one event per line):
        col 0-11 : 12 float features  |  col 12 : weight  |  col 13 : ignored

Amplitude embedding:
    Qiskit StatePreparation produces a circuit encoding the amplitude vector.
    The circuit is decomposed into primitive 1- and 2-qubit gates, converted to
    a pytket Circuit via qiskit_to_tk, rebased to {Rx, Rz, CZ} via AutoRebase,
    and converted to a list of PyQuil instruction objects via tk_to_pyquil.
    The instruction list is built once per event and reused across all passes.

Training pipeline (mini-batch mode):
    Events are divided into batches of BATCH_SIZE. One sweep through all batches
    is one N_iter pass.

    Per batch:
      1. Run all BATCH_SIZE embedded circuits at current theta -> P(|1>) per event.
      2. Compute batch loss (weighted binary cross-entropy, sum over batch events only).
      3. Compute batch gradient via parameter shift rule.
      4. One Adam step (sklearn AdamOptimizer) -> update theta.
...
from pyquil.gates import I, RESET  # I is used only as a no-op guard for the |0000> edge case
...
# This implementation rebases the embedding to {Rx, Rz, CZ} (native Rigetti gates),
# so switching to a real QPU is supported without additional decomposition steps.
...
parser = argparse.ArgumentParser(
    description="PQC mini-batch training for HEP classification (PyTket). "
                "All arguments optional; defaults from USER SETTINGS."
)
...
if circuit_number < 1 or circuit_number >= len(clib.CIRCUITS) or clib.CIRCUITS[circuit_number] is None:
    print(f"Error: circuit_number {circuit_number} invalid.")
    sys.exit(1)
    ...
    with open(filepath) as f:
        for line in f:
            ...
            try:
                ...
                skipped += 1
    if skipped: print(f"  Skipped {skipped} non-event lines in '{filepath}'")
...
if BALANCE_CLASSES and max_events is not None:
    ...
    if n_each > available:
        ...
        print(f"Error: BALANCE_CLASSES: only {available} {smaller} events available.")
        sys.exit(1)
    ...
    if max_events is not None and max_events > len(all_feats):
        print(f"Error: MAX_EVENTS={max_events} exceeds {len(all_feats)}.")
        sys.exit(1)
...
# QUANTUM PIPELINE
# =============================================================================
# Defines all quantum operations: pytket-converted amplitude embedding (Qiskit
# StatePreparation rebased to Rx/Rz/CZ), measurement, loss, and gradient via the
...
def preprocess_event(x_raw):
    ...
    if norm < 1e-12: raise ValueError("Feature vector has zero norm.")
    return x_padded / norm


# --- Amplitude embedding via Qiskit -> pytket -> PyQuil ----------------------

def embed_state_in_pyquil(amplitudes):
    """
    Convert the amplitude vector to a list of PyQuil instruction objects via:
      Qiskit StatePreparation -> decompose(reps=2) -> qiskit_to_tk
      -> DecomposeBoxes + AutoRebase({Rx, Rz, CZ}) -> tk_to_pyquil
    The instruction list is stored once per event and reused across all passes.
    If the state is |0000>, Identity gates are added to keep the program non-empty.
    """
    qc = QuantumCircuit(N_QUBITS)
    qc.append(StatePreparation(amplitudes.tolist(), normalize=False), range(N_QUBITS))
    # reps=2: StatePreparation wraps sub-circuits in boxes; two decompose passes
    # are needed to reach primitive gates that qiskit_to_tk can convert.
    qc_decomposed = qc.decompose(reps=2)

    tk_circ = qiskit_to_tk(qc_decomposed)
    DecomposeBoxes().apply(tk_circ)
    AutoRebase({OpType.Rx, OpType.Rz, OpType.CZ}).apply(tk_circ)
    p = tk_to_pyquil(tk_circ)

    if np.allclose(amplitudes[1:], 0.0):
...
def run_circuit_and_measure(embed_instrs, theta):
    """
    Build a fresh PyQuil Program from the stored instruction list, apply the
    variational form at the given theta, run Nm shots, and return P(qubit_0 = |1>).
    """
    p = Program()
    p += RESET()
    for instr in embed_instrs:
        p += instr
...
def compute_batch_gradient_and_loss(batch_embeddings, batch_labels, batch_weights, theta,
                                    shift_rules):
    """
    Compute gradient of batch loss w.r.t. theta and the batch loss value.
    Forward pass (p_current) is used for both the loss and the analytical dL/dp.
    Per theta component k the correct exact shift rule is applied:
      two_term  : grad[k] = (1/N) * dot(dL_dp, (p+ - p-) / 2)   [π/2 shift]
      four_term : grad[k] = (1/N) * dot(dL_dp, d1*(fa+−fa−) − d2*(fb+−fb−))
                  d1=1/2, d2=(√2−1)/4, a-shifts ±π/2, b-shifts ±π.
    ...
    p_current = np.array([
        run_circuit_and_measure(instrs, theta) for instrs in batch_embeddings
    ])

    batch_loss = compute_loss(p_current, batch_labels, batch_weights)

    p_clip = np.clip(p_current, 1e-7, 1 - 1e-7)
    dL_dp  = batch_weights * (-batch_labels / p_clip + (1 - batch_labels) / (1 - p_clip))
    ...
    for k in range(len(theta)):
        if shift_rules[k] == 'two_term':
            ...
            tm    = theta.copy()
            tm[k] -= np.pi / 2
            p_plus  = np.array([run_circuit_and_measure(instrs, tp) for instrs in batch_embeddings])
            p_minus = np.array([run_circuit_and_measure(instrs, tm) for instrs in batch_embeddings])
            ...
            f_ap = np.array([run_circuit_and_measure(instrs, ta_p) for instrs in batch_embeddings])
            f_am = np.array([run_circuit_and_measure(instrs, ta_m) for instrs in batch_embeddings])
            f_bp = np.array([run_circuit_and_measure(instrs, tb_p) for instrs in batch_embeddings])
            f_bm = np.array([run_circuit_and_measure(instrs, tb_m) for instrs in batch_embeddings])
...
def evaluate_global_loss(all_embeddings, event_labels, event_weights, theta, desc="Eval"):
    ...
    p_values = np.array([
        run_circuit_and_measure(instrs, theta)
        for instrs in tqdm(all_embeddings, desc=f"  {desc}", unit="event", leave=False)
    ])
...
class CircuitRecorder:
    """Wraps a PyQuil Program to record gate instructions from the variational form."""
...
def record_final_circuit(theta_final):
    """Replay the variational form with theta_final over a dummy |0000> embedding to record the trained gate sequence."""
    dummy_amp    = np.zeros(N_DIM, dtype=complex)
    dummy_amp[0] = 1.0
    embed_instrs = embed_state_in_pyquil(dummy_amp)
    base_prog    = Program()
    base_prog   += RESET()
    for instr in embed_instrs:
        base_prog += instr
...
def generate_theta_labels(gate_sequence):
    ...
    for gate in gate_sequence:
        if gate['params']:
            ...
            val     = gate['params'][0]
            if is_ctrl: out.append(f"CTRL-{name}(ctrl=q{ctrl}, tgt=q{target}) = {val:.6f} rad")
            else:        out.append(f"{name}(q{target}) = {val:.6f} rad")
    return out
...
# TRAINING SETUP
# =============================================================================
# Pre-computes one pytket-converted embedding per event (Rx/Rz/CZ gate list),
# initialises theta, and creates the sklearn Adam optimiser.


# Pre-compute one pytket embedding per event (list of PyQuil instruction objects).
# All N_iter passes reuse these cached instruction lists, eliminating repeated construction.
print(f"Pre-computing pytket embeddings ({N_EVENTS} events)...")
all_embeddings = []
...
if global_do_start or batch_do_start:
    ...
    if batch_do_start:
        p_start = np.array([
            run_circuit_and_measure(instrs, theta_current)
            for instrs in tqdm(all_embeddings[:batch_size], desc="  Batch  [start]", unit="event", leave=False)
        ])
...
for current_iter in range(1, n_iter + 1):
    ...
    with tqdm(range(N_BATCHES_PER_ITER),
        ...
        for batch_idx in batch_bar:
            ...
            grad, current_batch_loss = compute_batch_gradient_and_loss(
                pass_embeddings[start:end], pass_labels[start:end], pass_weights[start:end],
                theta_current, shift_rules
            )
            # Adam step.
    ...
    if current_iter in batch_ckpt_set or current_iter in global_ckpt_set:
        ...
        if current_iter in global_ckpt_set:
            ...
            _g_str = f"{g_loss:.6f}"
        tqdm.write(f"  [n={current_iter:{label_width}d}]  batch = {_b_str}   GLOBAL = {_g_str}")
...
classifier_script = textwrap.dedent(f"""\
    # classifier.py  —  standalone classifier generated by PQC_HEP_training_file_pytket.py
    #
    ...
    #
    # Embedding: Qiskit StatePreparation -> pytket (decompose + rebase to {{Rx,Rz,CZ}}) -> PyQuil.
    # This native-gate circuit is hardware-compatible; if {QUANTUM_COMPUTER} is a real Rigetti
    # QPU name (e.g. "Ankaa-2"), this classifier runs on real hardware without modification.
    # For the QVM simulator, start the containers before running:
    #   docker run --rm -p {QVM_PORT}:5000   rigetti/qvm   -S
    #   docker run --rm -p {QUILC_PORT}:5555 rigetti/quilc -R
    #
    # Usage:
    ...
    #
    # Requirements: qiskit, pytket, pytket-qiskit, pytket-pyquil, pyquil (QVM running), numpy
    ...
    from pyquil.gates import RX, RY, RZ, H, CNOT, CZ, I, RESET
    ...
    def embed_state_in_pyquil(amplitudes):
        # Encode the amplitude vector via Qiskit StatePreparation, transpile through pytket
        # to native Rigetti gates {{Rx, Rz, CZ}}, then convert to a PyQuil Program.
        qc = QuantumCircuit(N_QUBITS)
        qc.append(StatePreparation(amplitudes.tolist(), normalize=False), range(N_QUBITS))
        qc_decomposed = qc.decompose(reps=2)
        tk_circ       = qiskit_to_tk(qc_decomposed)
        DecomposeBoxes().apply(tk_circ)
        AutoRebase({{OpType.Rx, OpType.Rz, OpType.CZ}}).apply(tk_circ)
        tk_p = tk_to_pyquil(tk_circ)
        p = Program()
        p += RESET()
        for instr in tk_p.instructions:
            p += instr
        # Guard against all-zero amplitude (edge case: adds identity gates to keep the circuit valid).
        if np.allclose(amplitudes[1:], 0.0):
            for i in range(N_QUBITS):
                p += I(i)
        return p
...
{gate_lines_cls}
    ...
    # Connect once and reuse for all events (works for both QVM and real Rigetti QPU).
    os.environ["QCS_SETTINGS_APPLICATIONS_QVM_URL"]   = f"http://127.0.0.1:{{QVM_PORT}}"
    os.environ["QCS_SETTINGS_APPLICATIONS_QUILC_URL"] = f"tcp://127.0.0.1:{{QUILC_PORT}}"
    qvm = get_qc("{QUANTUM_COMPUTER}")
