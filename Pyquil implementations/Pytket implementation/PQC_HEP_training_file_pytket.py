...
Description:
    ...
    Uses Qiskit StatePreparation -> pytket -> PyQuil for amplitude embedding.
...
Training pipeline (mini-batch mode):
    ...
    Per batch:
      ...
      2. Compute batch loss (NN.pdf eq. 3, sum over batch events only).
...
classifier_script = textwrap.dedent(f"""\
    # classifier.py
    # Standalone classifier — generated by PQC_HEP_training_file_pytket.py
    # Run timestamp  : {timestamp}
    # Circuit        : [{circuit_number}] {circuit_description}
    # Layers         : {n_layers}  |  Parameters: {N_PARAMS}
    # Requires: qiskit, pytket, pytket-qiskit, pytket-pyquil, pyquil (QVM running), numpy
    ...
    def preprocess(x_raw):
        x_padded = np.concatenate([x_raw, np.zeros(N_DIM - N_FEAT)])
        norm = np.linalg.norm(x_padded)
        if norm < 1e-12: raise ValueError("Zero-norm feature vector.")
        return x_padded / norm
...
{gate_lines}
    ...
    qvm = get_qc("{QUANTUM_COMPUTER}")
    p.wrap_in_numshots_loop(Nm_classify)
    result = qvm.run(p.measure_all()).get_register_map()["ro"]
    p1     = float(np.array(result)[:, 0].mean())

    print(f"Classification probability = {{p1:.4f}}")
    print(f"Classification result      = {{'SIGNAL' if p1 >= 0.5 else 'BACKGROUND'}}")
""")
